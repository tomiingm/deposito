...
// ── Modal de aviso genérico (reemplaza alert() nativo del navegador) ──
...
const APP_ALERT_COLORS = {
    info: { bg: 'var(--color-ice)', fg: 'var(--color-blue)' },
    warning: { bg: 'rgba(245, 158, 11, 0.14)', fg: 'var(--color-warning)' },
    error: { bg: 'rgba(239, 68, 68, 0.12)', fg: 'var(--color-danger)' },
    success: { bg: 'rgba(34, 197, 94, 0.12)', fg: 'var(--color-success)' }
};

const APP_ALERT_TITLES = {
    info: 'Información',
    warning: 'Atención',
    error: 'Error',
    success: 'Listo'
...
/**
 * Reemplazo estético de window.alert(). Uso: showAppAlert('Mensaje', 'warning')
 * Tipos disponibles: 'info' (default), 'warning', 'error', 'success'.
 */
function showAppAlert(message, type, title) {
    const overlay = document.getElementById('app-alert-overlay');
    if (!overlay) {
        // Fallback por si el modal global no está presente en esta página
        window.alert(message);
        return;
    }

    const t = APP_ALERT_COLORS[type] ? type : 'info';
    const colorSet = APP_ALERT_COLORS[t];
    ...
    if (iconEl) {
        ...
        iconEl.style.color = colorSet.fg;
        iconEl.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${APP_ALERT_ICONS[t]}</svg>`;
    }
    if (titleEl) titleEl.textContent = title || APP_ALERT_TITLES[t];
