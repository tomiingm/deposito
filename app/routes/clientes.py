...
def listar_clientes():
    ...
    try:
        ...
        if query:
            count_sql += " AND (nombre LIKE %s OR CAST(id_cliente AS CHAR) LIKE %s)"
            like_str = f"%{query}%"
            count_params.extend([like_str, like_str])
        ...
        if query:
            sql += " AND (c.nombre LIKE %s OR CAST(c.id_cliente AS CHAR) LIKE %s)"
            like_str = f"%{query}%"
            select_params.extend([like_str, like_str])

        sql += " GROUP BY c.id_cliente, c.nombre, c.activo ORDER BY c.nombre ASC LIMIT %s OFFSET %s"
...
def nuevo_cliente():
    ...
    if request.method == 'POST':
        ...
        if not nombre:
            ...
            return render_template('clientes/nuevo_cliente.html', nombre=nombre)
        ...
        try:
            ...
            if existente:
                if existente['activo'] == 1:
                    ...
                    cursor.execute("UPDATE cliente SET activo = 1 WHERE id_cliente = %s", (existente['id_cliente'],))
                    conn.commit()
                    flash(f"El cliente '{nombre}' estaba dado de baja y fue reactivado exitosamente.", "success")
                    return redirect(url_for('clientes.listar_clientes'))

            cursor.execute("INSERT INTO cliente (nombre, activo) VALUES (%s, 1)", (nombre,))
            conn.commit()
    ...
    return render_template('clientes/nuevo_cliente.html', nombre='')
...
def editar_cliente(id_cliente):
    ...
    if request.method == 'POST':
        ...
        if not nombre:
            flash("El nombre o razón social es obligatorio.", "error")
            return render_template('clientes/editar_cliente.html', cliente={'id_cliente': id_cliente, 'nombre': nombre})
        ...
        try:
            ...
            cursor.execute("UPDATE cliente SET nombre = %s WHERE id_cliente = %s", (nombre, id_cliente))
            conn.commit()
    ...
    try:
        cursor.execute("SELECT id_cliente, nombre, activo FROM cliente WHERE id_cliente = %s", (id_cliente,))
